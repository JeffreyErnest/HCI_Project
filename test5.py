        ...
        with mp_hands.Hands(min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands:
            ...
            while cap.isOpened():
                ...
                if drawing_mode == "nose" and results_face.multi_face_landmarks:
                    for face_landmarks in results_face.multi_face_landmarks:
                        nose_landmark = face_landmarks.landmark[1]
                        # nose_position = (WIDTH - int((nose_landmark.x *nose_landmark.x) * WIDTH), int((nose_landmark.y * nose_landmark.y) * HEIGHT))
                        noseX = max(0, min(1, nose_landmark.x))
                        noseY = max(0, min(1, nose_landmark.y))
                        shiftX = 1
                        shiftY = 1
